...
Well, this is a timestamp. Namely, this is then numer of 100-nanoseconds intervals since October 15, 1582 at midnight UTC.
...
This is the same date as the input value! Because of the precision of 12 bytes, we're able to track the input time down to the closest 100th-nanosecond.
...
UUIDv1 has 3 bytes for `Clock` and UUIDv2 has only 1 byte for `Clock`. The remaining two bytes of Clock has been moved to `Local Domain` in UUIDv2.

If you recall, the `Clock` is short for "Clock Sequence" and is a random number generated by the system's clock.

In UUIDv1, the clock sequence is a 14-bit number, which can host a value from `0` to `16,383`.
...
This means that **UUIDv1 generators can create `16,384` unique IDs until the timestamp increments, while UUIDv2 generators can only generate `64` unique IDs until the timstamp increments**.

This is made substancially worse when you remember that _UUIDv1 generators increment their timestamps every 100 nanoseconds while UUIDv2 increments every 7 minutes._
...
Hopefully this has been insightful as to why most people tend to avoid UUIDv2 in production applications. Collisions are the bane of every unique ID system, and UUIDv2 has a pretty rough time of mitigating that.
