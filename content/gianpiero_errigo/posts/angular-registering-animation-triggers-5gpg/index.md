...
originalLink: "https://dev.to/playfulprogramming-angular/angular-registering-animation-triggers-5gpg",
...
We already seen factory's [**createRenderer**](https://github.com/angular/angular/blob/main/packages/platform-browser/animations/src/animation_renderer.ts#L44) expecting a *type* argument, declared as [**RendererType2**](https://angular.io/api/core/RendererType2).
...
- **id**: this is an identifier generated by compiler for the specific Component **class**.
  ...
  The property has to be considered as a *static class field*: **every instance of the same @Component class will get the same id**.
  ...
  This definition loses its accountability when talking about *AnimationRenderer*, where two different instances of the same component will get assigned two different instances of the renderer, due to the introduction of *namespace* concept. (More about this later)
...
- **data**: this property shaped as an open dictionary could sound "alien", but it's actually the one used to define a whole set of optional characteristics of the component that's gonna be created.
  It's crucial for our case study because that's where *AnimationRenderer* will look for declared [**AnimationTriggerMetadata**](https://angular.io/api/animations/AnimationTriggerMetadata) to be registered.
