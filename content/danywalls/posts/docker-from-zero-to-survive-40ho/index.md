...
_Hey Docker, please run nginx using port x and translate port x to port y from nginx_; the command version is `docker run -p 8080:80 nginx`.
...
*ENV* declare variables, e.g., ENV NODE_ENV=production.
...
- depends_on: define a container dependency.
...
depends_on: if our container requires another before starting.
