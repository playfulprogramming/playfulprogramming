...
export function SearchPageBase() {
	...
	const fetchSearchQuery = useCallback(({ signal, queryKey: [_, query] }: { signal: AbortSignal, queryKey: [string, SearchQuery]}) => {
		plausible && plausible("search", { props: { searchVal: query.searchQuery } });
	  
		const postsOffset = (query.searchPage - 1) * MAX_POSTS_PER_PAGE;
		const collectionsOffset = (query.searchPage - 1) * MAX_COLLECTIONS_PER_PAGE;
    ...
    const showCollections = query.display === "all" || query.display === "collections";
    ...
    const collectionsPageCount = Math.ceil(data.totalCollections / MAX_COLLECTIONS_PER_PAGE);

    // Use the larger page count when showing both
