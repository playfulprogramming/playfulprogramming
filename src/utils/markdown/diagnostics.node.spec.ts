...
describe("Markdown diagnostics reporting", () => {
	...
	it("reports messages from multiple plugins before rethrowing a fatal diagnostic", async () => {
		...
		const processor = unified()
			...
			.use(() => (_, file) => {
				file.message("Missing image", "images:missing-image");
				file.message("Invalid link", "links:invalid-link");
				file.fail("Cannot render content", "renderer:invalid-content");
			});
		await expect(
