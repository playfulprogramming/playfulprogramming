...
export const getDatas = () => {
    const unicornsRaw: Array<Omit<UnicornInfo, 'roles' | 'pronouns'> & {
        roles: string[],
        pronouns: string
    }> = JSON.parse(fs.readFileSync(join(dataDirectory, 'unicorns.json')).toString())

    const rolesRaw: RolesEnum[] = JSON.parse(fs.readFileSync(join(dataDirectory, 'roles.json')).toString())

    const pronounsRaw: PronounInfo[] = JSON.parse(fs.readFileSync(join(dataDirectory, 'pronouns.json')).toString())
