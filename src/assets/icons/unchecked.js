import React, {useRef} from 'react';
import { genId } from "../../utils/a11y/getNewId"

export const Unchecked = (props) => {
    const clipId = useRef(`path-1-inside-1${genId()}`)

    return <svg width="36" height="36" viewBox="0 0 36 36" fill="none" xmlns="http://www.w3.org/2000/svg" {...props}>
        <mask id={clipId.current} fill="white">
            <path fillRule="evenodd" clipRule="evenodd"
                  d="M27 6H9C7.34315 6 6 7.34315 6 9V27C6 28.6569 7.34315 30 9 30H27C28.6569 30 30 28.6569 30 27V9C30 7.34315 28.6569 6 27 6ZM9 3C5.68629 3 3 5.68629 3 9V27C3 30.3137 5.68629 33 9 33H27C30.3137 33 33 30.3137 33 27V9C33 5.68629 30.3137 3 27 3H9Z"/>
        </mask>
        <path fillRule="evenodd" clipRule="evenodd"
              d="M27 6H9C7.34315 6 6 7.34315 6 9V27C6 28.6569 7.34315 30 9 30H27C28.6569 30 30 28.6569 30 27V9C30 7.34315 28.6569 6 27 6ZM9 3C5.68629 3 3 5.68629 3 9V27C3 30.3137 5.68629 33 9 33H27C30.3137 33 33 30.3137 33 27V9C33 5.68629 30.3137 3 27 3H9Z"
              fill="#153E67"/>
        <path
          d="M9 9H27V3H9V9ZM9 9V9V3C5.68629 3 3 5.68629 3 9H9ZM9 27V9H3V27H9ZM9 27H9H3C3 30.3137 5.68629 33 9 33V27ZM27 27H9V33H27V27ZM27 27V33C30.3137 33 33 30.3137 33 27H27ZM27 9V27H33V9H27ZM27 9H33C33 5.68629 30.3137 3 27 3V9ZM6 9C6 7.34315 7.34315 6 9 6V0C4.02944 0 0 4.02944 0 9H6ZM6 27V9H0V27H6ZM9 30C7.34315 30 6 28.6569 6 27H0C0 31.9706 4.02944 36 9 36V30ZM27 30H9V36H27V30ZM30 27C30 28.6569 28.6569 30 27 30V36C31.9706 36 36 31.9706 36 27H30ZM30 9V27H36V9H30ZM27 6C28.6569 6 30 7.34315 30 9H36C36 4.02944 31.9706 0 27 0V6ZM9 6H27V0H9V6Z"
          fill="#153E67" mask={`url(#${clipId.current})`}/>
    </svg>
}
