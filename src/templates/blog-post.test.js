...
test("Blog post page handles two authors", async () => {
  const { baseElement, findByText, findByTestId } = render(getElement(MockMultiAuthorPost))
  ...
  // Renders the post body properly
  expect((await findByTestId('post-body-div')).innerHTML).toBe('<div>Hello, friends</div>');
})
