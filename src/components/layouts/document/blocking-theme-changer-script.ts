...
import { serializeInlineCall } from "#utils/serialize-inline-call.ts";
...
const setColorsByTheme = serializeInlineCall(
	({
		...
		lightTheme: string;
		darkTheme: string;
		colorSchemeMediaQuery: string;
	}): void => {
		let mediaQuery: MediaQueryList | undefined;
		try {
			mediaQuery = window.matchMedia(colorSchemeMediaQuery);
		} catch {
			// Browsers without matchMedia fall back to light mode.
		}
...
const setBrandColorsByTheme = serializeInlineCall(
	({
		storageKey,
		properties,
		fontFamilies,
		...
		try {
			...
			Object.entries(theme).forEach(([storedProperty, storedValue]) => {
				const property = storedProperty.startsWith("--")
					? storedProperty
					: `--${storedProperty}`;
				if (
				...
				let cssValue = value;
				if (fontProperties.has(property)) {
					if (
						value &&
						!Object.prototype.hasOwnProperty.call(fontFamilies, value)
					) {
						return;
					}
					cssValue = value ? fontFamilies[value] : "";
				} else if (value) {
					if (!numberPattern.test(value)) return;
					const number = Number(value);
					if (
						!Number.isFinite(number) ||
						!hueProperties.has(property) ||
						number < 0 ||
						number > 360
					) {
						return;
			...
			// Ignore malformed saved theme values.
		}
	},
);

const setContrastByTheme = serializeInlineCall(
	({ storageKey }: { storageKey: string }): void => {
		...
		const preference =
			savedPreference === "more" || savedPreference === "less"
				? savedPreference
				: "system";
		const root = document.documentElement;
		root.classList.toggle("contrast-more", preference === "more");
		root.classList.toggle("contrast-less", preference === "less");
	},
);
...
const inlineScript = [
	setColorsByTheme({
		...
		colorSchemeMediaQuery: COLOR_SCHEME_MEDIA_QUERY,
	}),
	setContrastByTheme({ storageKey: CONTRAST_MODE_STORAGE_KEY }),
	setBrandColorsByTheme({
		storageKey: BRAND_THEME_STORAGE_KEY,
		properties: BRAND_THEME_PROPERTIES,
		fontFamilies: THEME_FONT_FAMILIES,
	}),
].join("");

// this must live in a module instead of front matter or it will be evaluated on every import instead of once. That makes the build very sad :(
export const blockingThemeChangerScript = (await Terser.minify(inlineScript))
	.code!;
