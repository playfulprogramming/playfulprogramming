...
import { getUserProfilePic } from "components/user-profile-pic/get-user-profile-pic";
...
export const getPostCardHTML = ({
	...
	const userProfilePicHTML = getUserProfilePic({
		authors: authorsMeta,
		unicornProfilePicMap,
        ...
        <li class="${cardStyles.card} ${className}" role="listitem">
            <div class="${cardStyles.cardContents}">
                ...
                <p class="${cardStyles.authorName}">
                    <span>by&nbsp;</span>
                    <a class="${cardStyles.authorLink}" href="/unicorns/${
		authorsMeta[0].id
	}">
                        ${authorsMeta[0].name}
                    </a>
                ...
                <div class="${cardStyles.dateTagSubheader}">
                    ...
                    ${tags
											.map(
												(tag) => `<span class="${cardStyles.tag}">${tag}</span>`
											)
											.join("")}
                </div>
                <p class="${cardStyles.excerpt}">${description || excerpt}</p>
            </div>
            ${userProfilePicHTML}
        </li>
`;
